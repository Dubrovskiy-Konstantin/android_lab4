...
class Maze1(val max_X: Float,val max_Y: Float) : IDrawable{
    ...
    init {
        ...
        rects.add(RectF(0f, 350f,350f, 450f))
    ...
    override fun contactPlayer(player: Player): Boolean {
        rects.forEach {
            if(it.intersects(player.hitBox.left,player.hitBox.top, player.hitBox.right, player.hitBox.bottom))
                return true
        }
        return false
    }
...
class Enemy1: IDrawable{
    val points = ArrayList<PointF>()
    val hitBox: RectF
    init {
        points.add(PointF(570f,1050f))
        points.add(PointF(680f,990f))
        points.add(PointF(680f,1090f))
        hitBox = RectF(570f, 990f, 680f, 1090f)
    }
    ...
    override fun contactPlayer(player: Player): Boolean {
        return hitBox.intersects(player.hitBox.left,player.hitBox.top, player.hitBox.right, player.hitBox.bottom)
    }
}

class Enemy2(speed: Float): IMovable{
    val hitBox: RectF
...
class Finish(playerR: Float): IDrawable{
    val hitBox: RectF
    private var x : Float
    private var y : Float
    private var r : Float
    ...
    override fun contactPlayer(player: Player): Boolean {
        return hitBox.contains(player.hitBox)
    }
}